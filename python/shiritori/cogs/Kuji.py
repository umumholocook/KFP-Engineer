...
from data.lungshan import LUNGSHAN
...
class Kuji(commands.Cog):
    ...
    async def draw_ls(self, ctx:commands.Command, *argv):
        ...
        ls = LUNGSHAN[index]
        status = ls["status"]
        img = discord.File(KujiUtil.getImageUrlLs(status), filename=KujiUtil.getImageNameLs(status))
        await ctx.reply(file=img, embed=self.createEmbededLs(ls, datetime.now()))
        self.db.updateMemberLs(ctx.author.id, index)
    ...
    async def get_history(self, ctx:commands.Command, *argv):
        ...
        historyLs = self.db.getHistoryLs(ctx.author.id)
    ...
    def createEmbededLs(self, ls, timestamp):
        status = ls["status"]
        title = self.getTitle(timestamp)
