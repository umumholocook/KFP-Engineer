...
class SuperChatMeme(commands.Cog):
    ...
    @app_commands.command(name="超級留言", description="<金額> <@對象> <留言>")
    async def superchat_group(self, interaction: discord.Interaction, sc_money: int = -1, user: User = None, message: str = ""):
        ...
        avatar = await DiscordUtil.read_avatar_image(interaction.user)
    ...
    @app_commands.command(name="超級留言說明", description="SC(超級留言)功能說明")
    ...
    async def show_help_message(interaction: discord.Interaction):
        ...
        msg += "/超級留言 <金額> <@對象> <留言> 給該對象多少硬幣，後面文字可留言(中間不可有空白)\n"
